"""
SmartDocAI FastAPI Backend — Vercel Serverless
Uses /tmp for SQLite (ephemeral, resets on cold start).
"""
...
def init_db():
    ...
    c.executescript("""
        CREATE TABLE IF NOT EXISTS resumes (
            ...
            filename TEXT NOT NULL, content TEXT,
            summary TEXT, top_words TEXT,
...
def top_words(text, n=10):
    ...
    return [w for w, _ in Counter(w for w in words if w not in STOPWORDS).most_common(n)]

def make_summary(text, n=5):
...
async def upload_resume(file: UploadFile = File(...)):
    ...
    cur.execute("INSERT INTO analytics_events (event_type,detail) VALUES(?,?)",
                ("resume_upload", file.filename))
    conn.commit()
    conn.close()
    return {"id": new_id, "filename": file.filename, "summary": sm,
            "top_words": tw, "used_fallback": True, "uploaded_at": now}
...
def get_insights(limit: int = 50, id: Optional[int] = None):
    ...
    if id is not None:
        ...
        r = dict(row); r["top_words"] = json.loads(r.get("top_words") or "[]")
        r["used_fallback"] = bool(r["used_fallback"]); return r
    ...
    for row in rows:
        r = dict(row); r["top_words"] = json.loads(r.get("top_words") or "[]")
        r["used_fallback"] = bool(r["used_fallback"]); result.append(r)
    return result
...
def analytics_summary():
    ...
    cur.execute("SELECT event_type, COUNT(*) FROM analytics_events GROUP BY event_type ORDER BY 2 DESC")
    event_breakdown = {r[0]: r[1] for r in cur.fetchall()}
    cur.execute("SELECT DATE(created_at), COUNT(*) FROM analytics_events GROUP BY 1 ORDER BY 1 DESC LIMIT 7")
    daily_activity = [{"date": r[0], "count": r[1]} for r in cur.fetchall()]
    conn.close()
    return {"total_resumes": total_resumes, "total_events": total_events,
            "event_breakdown": event_breakdown, "daily_activity": daily_activity}
...
def submit_feedback(fb: FeedbackIn):
    init_db()
    conn = sqlite3.connect(DB_PATH)
    conn.execute("INSERT INTO feedback (feature,rating,comment) VALUES(?,?,?)",
